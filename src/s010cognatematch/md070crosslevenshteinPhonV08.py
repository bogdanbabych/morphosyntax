        ...
        for (SWordA, SPoSA, IFrqA, LTargetsEval) in LWordsA:
            ...
            for (STargetE, SPoSA) in sorted(DTargetsEval.keys(), reverse=False, key=lambda k: k[1][0]):
                if (STargetE, SPoSA) in DCognates.keys():
                    ...
                    LLSortCog = self.findRanks((STargetE, SPoSA), DCognates, [0,1])
                    for (ICogRank, ILenLSortCog, LSortCog) in LLSortCog:
                        ...
                        for (ICogRank, TKey, TVals) in LSortCog: # corrected
                            SKey = str(TKey)
                            SVals = str(TVals)
                            sys.stdout.write('\t%(ICogRank)d\t%(SKey)s\t%(SVals)s\n' % locals())
                        
                    (Lev0Norm, Lev1Norm, Lev0, Lev1) = DCognates[(STargetE, SPoSA)]
                    sys.stdout.write('%(STargetE)s\t%(SPoSA)s\t%(Lev0Norm)f\t%(Lev1Norm)f\t -- found \n' % locals())
                    sys.stdout.flush()
                else:
                    (Lev0NormE, Lev1NormE, Lev0E, Lev1E) = DTargetsEval[(STargetE, SPoSA)]
                    sys.stdout.write('%(STargetE)s\t%(SPoSA)s\t%(Lev0NormE)f\t%(Lev1NormE)f\t -- NOT found \n' % locals())
                    sys.stdout.flush()
